...
Figma 화면명세서 ver 0.25 전 캔버스를 텍스트로 추출해 노션 API 명세·구현과 대조했다. 위 목록 외에 **화면은 있으나 명세도 구현도 없는 것**이 아래만큼 더 있다. 상세는 각 도메인 노션 페이지의 `미구현` 절.
...
- **통계/분석** — 활성 장부, 예산 소진율 랭킹, 장부별 지출 비중(상위 4 + 기타).
- **증빙자료 앨범** — 모임 전체 증빙을 그리드로 모아 보고 원본 내역으로 역참조.
- **내역 화면 전반** — 모임 전체 내역 목록(장부 다중 필터 + 잔액 요약), 내역명·장부명 통합 검색, 담당자 필드.
- **폴더 화면 전반** — 폴더+장부 혼합 목록, 선택 이동(다중), 최상위 영역 조회.
- **대시보드** — 최근 14일 캘린더, 월간 캘린더, 마감 임박 회비 3건. 현재 응답은 화면과 거의 대응되지 않는다.
- **보고서 유형 분리** — 장부별(기간 없음 + 구분 필터) / 기간별(장부 없음). 현재는 넷 다 필수라 어느 화면도 못 만든다.
- **회비** — 시작일과 `SCHEDULED` 상태, 납부 상태 일괄 변경, 목록 검색.

구현과 화면명세가 **어긋나는** 것도 하나 있다: 회비 마감이 미납자가 있으면 `UNPAID_MEMBER_EXISTS(409)` 로 막히는데, 화면명세는 미납자가 남아도 마감하고 그때까지 모인 금액만 수입으로 기록한다. 이 제약이 남아 있으면 한 명이라도 끝내 안 내는 회비를 영원히 마감할 수 없다.
