...
- 파일은 S3 호환 Object Storage 예정이나, **현재 구현은 서버 디스크**(`billage.file.storage-path`, dev 기본 `./data/files`). 전환 시 `FileStorage` 구현체만 교체. DB는 주기적 덤프 → S3 백업.
...
→ ~~10. 증빙 파일~~ **(완료 — 단, 저장소는 서버 디스크. Object Storage 전환 미정)** → ~~11. 대시보드·통계~~ **(완료, 2026-08-16 — 회비 집계는 Dues 이후)** → 12. 보고서 → 13. 납부 관리 → 14. 프론트 연동 QA → 15. 런칭 서버 배포
